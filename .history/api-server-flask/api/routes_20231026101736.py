...
import jwt
from .models import db, User, Role, JWTTokenBlocklist
from .config import BaseConfig
import requests


rest_api = Api(version="1.0", title="LIDISKI API")
...
roles_edit_model = rest_api.model('UserEditRole', {"id": fields.String(required=True),
                                                 "roles": fields.List(cls= Role, required=True)
                                                 })
...
@rest_api.route('/api/users/roles')    
class EditUserRoles(Resource):
    """
       Edit user roles
    """

    @token_required
    @rest_api.expect(roles_edit_model)
    def post(self, current_user):
        req_data = request.get_json()
        
        _roles = req_data.get('roles')
        if not _roles:
            return {"success": False, "msg": "roles is empty"}, 400
        
        current_user.roles = _roles
        current_user.save()
